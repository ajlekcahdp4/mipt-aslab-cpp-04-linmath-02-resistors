...
template <typename T, typename U, typename t_comp = std::equal_to<T>, typename t_hash = std::hash<T>>
...
public:
  ...
  void insert_edge(std::pair<T, T> vert, U attr_fwd, U attr_bck) {
    ...
    if (m_comp(first_id, second_id)) throw std::invalid_argument{"Graph can't have self loops"};
  ...
  bool contains_edge(std::pair<T, T> vert) const { return lookup_edge(vert).has_value(); }

  std::optional<std::pair<adjecency_map_it, adjecency_map_it>> lookup_edge(std::pair<T, T> vert) {
    auto [first_id, second_id] = vert;

    if (m_comp(first_id, second_id)) return std::nullopt;

    auto first_found = m_vertices.find(first_id);
    auto second_found = m_vertices.find(second_id);

    if (first_found == m_vertices.end() || second_found == m_vertices.end()) return std::nullopt;
  ...
  std::optional<std::pair<const_adjecency_map_it, const_adjecency_map_it>> lookup_edge(std::pair<T, T> vert) const {
    auto [first_id, second_id] = vert;
  ...
  std::vector<self_type> connected_components() const {
    ...
    for (const auto &v : m_vertices) {
      ...
      for (const auto &p : v.second) {
        ...
        connected_representatives[found].insert_edge({v.first, p.first}, fwd_edge->second, bck_edge->second);
      }
    }
