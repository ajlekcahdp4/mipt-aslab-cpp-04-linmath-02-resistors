...
public:
  ...
  size_type vars() const {
    ...
    return std::max_element(m_equations.begin(), m_equations.end(), comp)->vars();
  }

  matrix<value_type> get_xtnd_matrix() const {
    if (!m_changed_flag) return m_matrix;

    auto               rows = size(), cols = vars() + 1;
    matrix<value_type> xtnd_matrix{rows, cols};

    for (unsigned i = 0; i < rows; ++i) {
      std::copy(m_equations[i].begin(), m_equations[i].end(), xtnd_matrix[i].begin());
    }
