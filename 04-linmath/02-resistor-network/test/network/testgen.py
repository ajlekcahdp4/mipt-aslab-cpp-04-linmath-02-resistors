...
def generate_matrix_rand_det_int(p_right: int, p_minsize: int, p_maxsize: int, p_shuffles: int, p_additions: int):
    ...
    for group in p_config['groups']:
        for c in range(group['number']):
            test_ans_list = generate_matrix_rand_det_int(
                group['diag']['right'], group['size']['min'], group['size']['max'], group['shuffles'], group['additions'])

            ans = test_ans_list[1]
            test = test_ans_list[0]

            save_test_ans(p_config['output_path'], generate_test_string(test), generate_ans_string(
                ans), c, group['test_fmt_string'], group['ans_fmt_string'])
